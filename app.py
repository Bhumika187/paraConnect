...
module_path = r"C:\Users\acer\Desktop\index\paraconnect.py"
...
class NextApp(App):
    def __init__(self, shared_dict, lock, **kwargs):
        ...
        self.center_x = 300
        self.center_y = 250  
        self.last_press_timing = time.time()
        self.keys = [
            ['R', 'D', 'Q', 'F', 'G', 'Y', 'J', 'K'],
            ['KMO', 'Z', 'X', 'C', 'V', 'B', 'U', 'M'],
            ['enter', 'W', 'backspace', 'P', '.', 'L', 'space', 'abc'],
            ['E', 'T', 'A', 'O', 'I', 'N', 'S', 'H']
        ]
    ...
    def setup_layout(self):
        ...
        if not self.shared_dict['keyboard_mode']:
            ...
            self.toggle_voice_btn = Button(text='Toggle Voice Mode',  on_press=self.toggle_voice_mode)
    ...
    def toggle_keyboard_mode(self, instance):
        self.shared_dict['keyboard_mode'] = True
        pyautogui.move(-300, 0)
        pyautogui.click()
        self.setup_layout()
        
    def toggle_voice_mode(self, instance):
            if not self.shared_dict['voice_mode']:
                subprocess.Popen(['python', 'paraconnect.py'])
    ...
    def draw_keyboard(self):
        angle = self.shared_dict['angle']
        segment = int(angle // (pi / 4))
        r = self.shared_dict['distance']*2
            ...
            for i in range(8):
                for j in range(4):
                    if self.keys[j][i]:
                        ...
                        if ring < 4 and self.keys[j][i] == self.keys[ring][segment]:
                            Color(0, 0, 1)
    ...
    def on_touch_down(self):
        ...
        if ring < 4:
            ...
            if key and self.shared_dict['blink'] and (time.time() - self.last_press_timing > 3):
                    if key == 'KMO':
    ...
    def comp_update(self, dt):
        self.message_label.text=self.shared_dict['msg']
        ...
        if ret:
            ...
            if results.multi_face_landmarks:
                ...
                if not self.shared_dict['keyboard_mode'] and self.shared_dict['distance'] > 50:
                    dx = int(12 * np.cos(self.shared_dict['angle']))
                    dy = int(12 * np.sin(self.shared_dict['angle']))
                    if not self.shared_dict['scroll']:
                        pyautogui.moveRel(dx, dy, duration=0.1)
                    else:
                        if 0 < self.shared_dict['angle'] < np.pi:
                            pyautogui.scroll(50)
                        else:
                            pyautogui.scroll(-50)
